...
void parse_label(parser*, shared_data* sd, label_t& ld, std::vector<VW::string_view>& words, reduction_features&)
{
  switch (words.size())
  {
    ...
    case 1:
      ...
      {
        // Allow floating-point labels (like 1.0, 2.0) by parsing as float then casting to int
        float label_float = float_of_string(words[0]);
        ld.label = (uint32_t)label_float;
      }
      ld.weight = 1.0;
      break;
    case 2:
