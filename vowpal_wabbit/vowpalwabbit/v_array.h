...
private:
  ...
  void reserve_nocheck(size_t length)
  {
    ...
    // Initialize new elements properly for both trivial and non-trivial types
    for (T* ptr = _end; ptr != _end_array; ++ptr) {
      new (ptr) T{};  // value initialization
    }
  }
...
void calloc_reserve(v_array<T>& v, size_t length)
{
  v.clear();
  v.reserve(length);
  // Initialize elements properly for both trivial and non-trivial types
  for (size_t i = 0; i < length; ++i) {
    v.push_back(T{});  // value initialization
  }
}
